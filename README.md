# Review Templates Showcase

A comprehensive collection of **35 stunning review/testimonial design templates** built with pure HTML, CSS, and JavaScript. Perfect for designers, developers, and anyone looking for inspiration or ready-to-use review components.

**[Live Demo](https://reviews-demo.vercel.app)**

![Review Templates Preview](preview.png)

## Features

- **35 Unique Designs** - From minimal to futuristic, classic to avant-garde
- **Pure HTML/CSS/JS** - No dependencies, no frameworks
- **Fully Responsive** - Works on all screen sizes
- **Dark & Light Themes** - Multiple background variations
- **Interactive Elements** - Carousels, sliders, hover effects
- **Hand-Drawn Stars** - Artistic outlined star ratings with organic feel
- **Modern Faded Palettes** - Soft pastels, earthy tones, muted colors
- **Copy & Customize** - Easy to extract and modify

## Template Styles

| # | Style | Theme | Description |
|---|-------|-------|-------------|
| 1 | Classic Grid | Light | Clean 3-column cards with hover effects |
| 2 | Masonry Layout | Purple Gradient | Variable height Pinterest-style |
| 3 | Testimonial Slider | Light | Large photo + quote with navigation |
| 4 | Futuristic Dark | Dark | Glassmorphic with neon accents |
| 5 | Glassmorphism | Ocean Gradient | Frosted glass blur effects |
| 6 | Minimal Clean | Light Gray | Ultra-simple list view |
| 7 | Magazine Editorial | White | Featured image + numbered sidebar |
| 8 | Bubble Chat | Soft Gradient | Conversation-style messages |
| 9 | Auto Carousel | Dark | Infinite horizontal scroll |
| 10 | Timeline | Warm Yellow | Chronological vertical layout |
| 11 | Grouped Reviews | Dark Slate | Categorized positive/constructive |
| 12 | Video Testimonials | White | Thumbnail cards with play buttons |
| 13 | Stats + Reviews | Blue Gradient | Rating breakdown combo |
| 14 | Social Proof Wall | Light | Multi-row auto-scrolling |
| 15 | Neon Cyberpunk | Black | Cyan/magenta cyber aesthetics |
| 16 | Peek Carousel | Light | Half-visible adjacent cards |
| 17 | Artistic Line Stars | Dark | Delicate outlined star ratings |
| 18 | Polaroid | Vintage | Instant-photo nostalgia |
| 19 | Brutalist | Monochrome | Raw, bold typography |
| 20 | Gradient Mesh | Colorful | Modern vibrant gradients |
| 21 | Editorial | Black | Big typography, art director style |
| 22 | Split Cards | White | Half image, half content |
| 23 | 3D Floating | Dark Blue | Perspective depth effects |
| 24 | Handwritten/Sketch | Paper | Hand-crafted doodle aesthetic |
| 25 | Stacked Cards | Black | Click-through card deck |

### Hand-Drawn Star Templates (Modern Flat & Faded)

| # | Style | Palette | Description |
|---|-------|---------|-------------|
| 26 | Soft Pastel | Cream/Peach | Gentle warmth with organic stars |
| 27 | Earthy Tones | Sage/Terracotta | Natural colors, left border accent |
| 28 | Faded Vintage | Sepia/Gray | Washed-out nostalgic feel |
| 29 | Nordic Minimal | Cool Gray | Scandinavian simplicity |
| 30 | Ink Wash | Charcoal/Cream | Asian-inspired brush aesthetics |
| 31 | Dusty Rose | Pink/Mauve | Warm blush tones |
| 32 | Sage & Stone | Green/Gray | Masonry with natural greens |
| 33 | Muted Ocean | Blue/Teal | Coastal calm with wave accents |
| 34 | Terracotta | Orange/Cream | Sun-baked warmth, horizontal scroll |
| 35 | Lavender Mist | Purple/Lilac | Soft ethereal purple dreams |

## Quick Start

1. Clone the repository:
```bash
git clone git@github.com:gabrielnejoiu/reviews-templates.git
```

2. Open `index.html` in your browser

3. Browse all 25 templates in one scrollable page

## Usage

Each section is self-contained with its own CSS styles and JavaScript. To use a template:

1. Find the section you like (e.g., `section-glass` for glassmorphism)
2. Copy the relevant CSS from the `<style>` block
3. Copy the HTML structure
4. Copy the JavaScript render function
5. Customize colors, fonts, and content

## Customization

### Colors
All colors are defined as CSS custom properties at the top of the file:
```css
:root {
    --primary: #6366f1;
    --accent: #f59e0b;
    --dark: #1e1e2e;
    /* ... */
}
```

### Fonts
The showcase uses three font families:
- **Inter** - Clean UI text
- **Playfair Display** - Elegant serifs
- **Space Grotesk** - Modern geometric

### Star Ratings
Three star styles are available:
- `createStars()` - Filled solid stars (classic yellow)
- `createLineStars()` - Outlined artistic stars (clean lines)
- `createHandDrawnStars()` - Organic imperfect stars (hand-crafted feel)

Each hand-drawn template uses its own color-coordinated star variant:
- `.star-handdrawn` - Warm pastel peach
- `.star-organic` - Earthy sage green
- `.star-vintage` - Faded sepia tone
- `.star-nordic` - Cool minimal gray
- `.star-ink` - Brushstroke charcoal
- `.star-rose` - Dusty pink
- `.star-sage` - Natural green
- `.star-ocean` - Muted teal
- `.star-terra` - Warm terracotta
- `.star-lavender` - Soft purple

## Browser Support

- Chrome (latest)
- Firefox (latest)
- Safari (latest)
- Edge (latest)

## License

MIT License - feel free to use in personal and commercial projects.

---

Made with creativity and code.
